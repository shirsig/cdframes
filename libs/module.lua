...
function global_mt:__index(key)
	...
	mutators = setmetatable({}, { __index=function(_, k) return function(v) if v == interface and (_G[k] == nil or error(nil)) then _G[k] = v else definition_helper.private[k] = v end end end})
	fields = setmetatable(
